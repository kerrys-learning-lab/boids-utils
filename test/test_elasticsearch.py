...
@pytest.mark.asyncio
async def test_index_save_new(test_elasticsearch: boids_utils.elastic):
    expected = create_test_session_config()
    test_elasticsearch.indices.session_configuration.save(expected)
    actual = test_elasticsearch.indices.session_configuration.get(
        expected.uuid)
...
@pytest.mark.asyncio
async def test_index_save_existing(test_elasticsearch: boids_utils.elastic):
    expected = create_test_session_config()
    test_elasticsearch.indices.session_configuration.save(expected)

    expected.state = boids_api.boids.SessionState.RUNNING
...
@pytest.mark.asyncio
async def test_index_search_default_pagination(test_elasticsearch: boids_utils.elastic):
    UNIQUE_TITLE = 'test_index_search_default_pagination'
    UNIQUE_NUM_BOIDS = 42
    expected_list = create_test_session_config(
        10, UNIQUE_TITLE, num_boids=UNIQUE_NUM_BOIDS)

    for expected in expected_list:
        boids_utils.elastic.indices.session_configuration.save(expected)

    results, pagination = test_elasticsearch.indices.session_configuration.search(title=UNIQUE_TITLE,
...
@pytest.mark.asyncio
async def test_index_search_odd_pagination(test_elasticsearch: boids_utils.elastic):
    ...
    original_pagination = boids_api.boids.Pagination(offset=0, limit=3)
...
@pytest.mark.asyncio
async def test_index_search_index_error_pagination(test_elasticsearch: boids_utils.elastic):
    UNIQUE_TITLE = 'test_index_search_odd_pagination'
    UNIQUE_NUM_BOIDS = 72
    expected_list = create_test_session_config(
        10, UNIQUE_TITLE, num_boids=UNIQUE_NUM_BOIDS)
    ...
    pagination = boids_api.boids.Pagination(offset=1000, limit=5)

    with pytest.raises(IndexError):
        test_elasticsearch.indices.session_configuration.search(title=UNIQUE_TITLE,
                                                                num_boids=UNIQUE_NUM_BOIDS,
